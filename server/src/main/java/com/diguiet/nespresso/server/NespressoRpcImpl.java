...
import com.diguiet.nespresso.server.nespresso.NespressoUtils;
import com.diguiet.nespresso.server.nespresso.model.Product;
import io.grpc.stub.StreamObserver;
import lombok.NonNull;
import sila2.com.diguiet.misc.nespresso.v1.NespressoGrpc;
import sila2.com.diguiet.misc.nespresso.v1.NespressoOuterClass;
...
public class NespressoRpcImpl extends NespressoGrpc.NespressoImplBase {
    ...
    private final Map<String, Product> capsules = NespressoUtils.getAvailableCoffees();
    ...
    public void addCoffees(
        ...
        if (!invalidCoffees.isEmpty()) {
            throw SiLAErrors.generateValidationError("Coffees", "Unknown coffees: " + invalidCoffees);
        }
    ...
    private static NespressoOuterClass.Get_Coffees_Responses.Coffees_Struct capsuleToCoffeesStruct(
            @NonNull final Product capsule
    ) {
        String headline = capsule.getHeadline();
        if (headline == null) {
            headline = "";
        }
        return NespressoOuterClass.Get_Coffees_Responses.Coffees_Struct
                ...
                .setDescription(SiLAFramework.String.newBuilder().setValue(headline).build())
